...
def KEY_DIC():
    key_dic = {}
    [key_dic.setdefault(str(x), 0) for x in range(256)]
    return key_dic
...
class Acquisition_Gadget(object):
    if __name__ == "__main__":
        ...
        # The TX data is: k, p, u, v
        ##################################################
        input_data_len = 4

        # The length of the RX data in Byte (B)
        # The RX data is: ((sbox(p+k)+v)+u)+v
        ##################################################
        output_data_len = 1
        ...
        # The data stored in trs file is: k, p, u, v, MaskedS(p=k+u)+u+v
        ...
        for i in tqdm(range(0, Number_traces)):
            ...
            # Generate inputs
            ##################################################
            k_int = secrets.randbits(8)  # type: int
            p_int = secrets.randbits(8)  # type: int
            u_int = secrets.randbits(8)  # type: int
            v_int = secrets.randbits(8)  # type: int

            for j, (key, value) in enumerate(key_dic.items()):
                if k_int == j:
                    # print("key_dic[key]", key_dic[key])
                    # print("val", value)
            ...
            u = u_int.to_bytes(1, sys.byteorder)
            v = v_int.to_bytes(1, sys.byteorder)
            mSB = GenMaskedSbox(u_int, v_int)
            ...
            # Retried data from scope to buffer assigned above
            ...
            data = bytearray(k + p + u + v)

            trs.write_trace(data, output, np.array(Databuffer), True)

            # Printing the data on screen after "step" times executions
            msb = mSB[p_int ^ k_int ^ u_int]
            int_msbuv = mSB[p_int ^ k_int ^ u_int] ^ u_int ^ v_int
            spkv = sbox[p_int ^ k_int] ^ v_int

            if msb != spkv:
                print("msb != spkv")
                break
            msbuv = int_msbuv.to_bytes(1, sys.byteorder)
            ...
            if i % step == 0:
                ...
                print('- v: {} '.format(v.hex()))
                print('- mSB1+v: {} '.format(msbuv.hex()))
                print('___________________________________________________________________')
            if int_msbuv != output[0]:
                print("msbuv != output")
                print("msbuv: ", (int_msbuv.hex()))
                print("output: ", output.hex())
                break
        ...
        #################################################
        trs.close()
    # pprint.pprint(key_dic, sort_dicts=False)
    # values = key_dic.values()
    # total = sum(values)
    # print(total)
    print("The number of traces:", i + 1)
    print('[+] Each trace contains {:d} samples'.format(samples))
